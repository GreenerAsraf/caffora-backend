...
async function main() {
    ...
    for (const category of categories) {
        const saved = await prisma_1.default.category.upsert({
    ...
    for (const product of products) {
        ...
        const saved = await prisma_1.default.product.upsert({
    ...
    await prisma_1.default.wishlist.upsert({
    ...
    const existingOrder = await prisma_1.default.order.findFirst({ where: { userId: user.id } });
    if (!existingOrder) {
